...
import type { InstitutionVerificationRow } from "@/lib/admin-verification-api";

export default function InstitutionVerificationTable() {
  const { data, isLoading, isError } = useInstitutionVerifications();
  const review = useReviewInstitutionVerification();
  ...
  const handlePickUp = (row: InstitutionVerificationRow) => {
    review.mutate(
      { id: row.id, action: "pick_up" },
      {
        onSuccess: () =>
          notify.success(`${row.institution_name}'s verification picked up.`),
        onError: (err) =>
          notify.error(
  ...
  const handleApprove = (row: InstitutionVerificationRow) => {
    review.mutate(
      { id: row.id, action: "approve" },
      {
        onSuccess: () => notify.success(`${row.institution_name} verified.`),
        onError: (err) =>
          notify.error(
    ...
    review.mutate(
      ...
      },
      {
        onSuccess: () => {
          notify.success(
            kind === "reject"
              ? `${row.institution_name}'s verification rejected.`
              : `Action requested from ${row.institution_name}.`,
          );
          setModal(null);
        },
        onError: (err) =>
    ...
    <div className="bg-white rounded-2xl border border-(--gray-200) px-5 py-4">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-(--gray-100)">
              ...
              <th className="text-[11px] font-semibold truncate tracking-widest text-(--gray-400) uppercase text-right pb-2">
                Action
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-(--gray-50)">
            {isLoading ? (
              ...
              <tr>
                ...
                >
                  No institution verifications submitted yet.
                </td>
              </tr>
            ) : (
              rows.map((row) => {
      ...
      {modal && (
        <ReasonModal
          ...
          description={
            modal.kind === "reject"
              ? `Explain why ${modal.row.institution_name}'s verification is being rejected.`
              : `Explain what ${modal.row.institution_name} needs to fix and resubmit.`
          }
