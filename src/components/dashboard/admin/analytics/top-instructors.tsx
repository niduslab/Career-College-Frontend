...
import { TOP_INSTRUCTORS } from "./data";

const maxRevenue = Math.max(...TOP_INSTRUCTORS.map((p) => p.revenueNum));

export default function TopInstructors() {
  return (
    <div className="bg-white rounded-2xl border border-(--gray-200) px-5 py-4 space-y-4">
      <p className="text-[14px] lg:text-[16px] font-semibold text-(--text-title)">Top Instructors</p>
      <div className="space-y-3">
        {TOP_INSTRUCTORS.map((p, i) => {
          ...
          return (
            <div key={p.name} className="flex items-center gap-3">
              ...
              <div className="w-7 h-7 rounded-full overflow-hidden shrink-0 bg-(--primary-50) text-(--primary-600) flex items-center justify-center text-[11px] font-semibold">
                {p.initials}
              </div>
