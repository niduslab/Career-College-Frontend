"use client";

import { useRef, useEffect } from "react";
import { Search, ChevronDown } from "lucide-react";
import type { AdminActionType } from "@/lib/admin-console-api";

const ACTIONS: AdminActionType[] = ["suspend", "reactivate", "role_change"];

const ACTION_LABEL: Record<AdminActionType, string> = {
  suspend: "Suspend",
  reactivate: "Reactivate",
  role_change: "Role Change",
};

interface AuditLogFilterBarProps {
  search: string;
  onSearchChange: (v: string) => void;
  action: AdminActionType | "All";
  onActionChange: (v: AdminActionType | "All") => void;
  actionOpen: boolean;
  onActionToggle: () => void;
}

export default function AuditLogFilterBar({
  search,
  onSearchChange,
  action,
  onActionChange,
  actionOpen,
  onActionToggle,
}: AuditLogFilterBarProps) {
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (!actionOpen) return;
    const handleClickOutside = (e: MouseEvent) => {
      if (ref.current && !ref.current.contains(e.target as Node)) {
        onActionToggle();
      }
    };
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [actionOpen, onActionToggle]);

  return (
    <div className="bg-white rounded-2xl border border-(--gray-200) px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
      <div className="relative flex-1 min-w-0">
        <Search className="w-4 h-4 text-(--gray-400) absolute left-3 top-1/2 -translate-y-1/2" />
        <input
          value={search}
          onChange={(e) => onSearchChange(e.target.value)}
          placeholder="Search by actor or target name..."
          className="lg:w-62.5 w-full h-9 pl-9 pr-3 rounded-lg border border-(--gray-200) text-[13px] text-(--text-title) placeholder:text-(--gray-400) focus:outline-none focus:ring-2 focus:ring-(--primary-200) focus:border-(--primary-300) transition-all"
        />
      </div>

      <div className="relative" ref={ref}>
        <button
          onClick={onActionToggle}
          className="text-[12px] cursor-pointer text-(--gray-600) border border-(--gray-200) rounded-lg px-3 py-2 flex items-center gap-1.5 hover:bg-(--gray-50) transition-colors"
        >
          {action === "All" ? "Action" : ACTION_LABEL[action]}
          <ChevronDown
            className={`w-4 h-4 transition-transform duration-200 ${actionOpen ? "rotate-180" : ""}`}
          />
        </button>
        {actionOpen && (
          <div className="absolute right-0 top-full mt-1 bg-white border border-(--gray-200) rounded-xl shadow-lg z-10 py-1 min-w-36">
            <button
              onClick={() => onActionChange("All")}
              className={`w-full text-left px-3 py-2 cursor-pointer text-[12px] transition-colors ${
                action === "All"
                  ? "bg-(--primary-50) text-(--primary-600) font-semibold"
                  : "text-(--gray-600) hover:bg-(--gray-50)"
              }`}
            >
              All Actions
            </button>
            {ACTIONS.map((a) => (
              <button
                key={a}
                onClick={() => onActionChange(a)}
                className={`w-full text-left px-3 py-2 cursor-pointer text-[12px] transition-colors ${
                  a === action
                    ? "bg-(--primary-50) text-(--primary-600) font-semibold"
                    : "text-(--gray-600) hover:bg-(--gray-50)"
                }`}
              >
                {ACTION_LABEL[a]}
              </button>
            ))}
          </div>
        )}
      </div>
    </div>
  );
}
