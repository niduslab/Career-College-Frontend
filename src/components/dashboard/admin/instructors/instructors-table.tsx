"use client";

import { useEffect, useMemo, useRef, useState } from "react";
import { Star } from "lucide-react";
import InstructorsFilterBar from "./filter-bar";
import RowActionsMenu from "./row-actions-menu";
import { Pagination } from "@/components/common/pagination";
import { INSTRUCTORS, InstructorStatus } from "./data";

const PAGE_SIZE = 6;

const STATUS_BADGE: Record<InstructorStatus, string> = {
  Active: "bg-emerald-50 text-emerald-600",
  Pending: "bg-blue-50 text-blue-600",
  Suspended: "bg-red-50 text-red-500",
};

export default function InstructorsTable() {
  const [search, setSearch] = useState("");
  const [status, setStatus] = useState<InstructorStatus | "All">("All");
  const [statusOpen, setStatusOpen] = useState(false);
  const [page, setPage] = useState(1);
  const [openMenuId, setOpenMenuId] = useState<string | null>(null);
  const menuRefs = useRef<Map<string, HTMLDivElement | null>>(new Map());

  useEffect(() => {
    function handleClick(e: MouseEvent) {
      if (openMenuId === null) return;
      const target = e.target as Node;
      const el = menuRefs.current.get(openMenuId);
      const insideWrapper = el?.contains(target) ?? false;
      const insidePortal = !!(target as HTMLElement).closest?.(
        "[data-action-portal]",
      );
      if (!insideWrapper && !insidePortal) setOpenMenuId(null);
    }
    document.addEventListener("mousedown", handleClick);
    return () => document.removeEventListener("mousedown", handleClick);
  }, [openMenuId]);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    return INSTRUCTORS.filter((i) => {
      const matchesSearch =
        !q ||
        i.name.toLowerCase().includes(q) ||
        i.email.toLowerCase().includes(q);
      const matchesStatus = status === "All" || i.status === status;
      return matchesSearch && matchesStatus;
    });
  }, [search, status]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const currentPage = Math.min(page, totalPages);
  const pageRows = filtered.slice(
    (currentPage - 1) * PAGE_SIZE,
    currentPage * PAGE_SIZE,
  );

  const updateAndResetPage =
    <T,>(setter: (v: T) => void) =>
    (v: T) => {
      setter(v);
      setPage(1);
    };

  return (
    <div className="space-y-4">
      <InstructorsFilterBar
        search={search}
        onSearchChange={updateAndResetPage(setSearch)}
        status={status}
        onStatusChange={(v) => {
          updateAndResetPage(setStatus)(v);
          setStatusOpen(false);
        }}
        statusOpen={statusOpen}
        onStatusToggle={() => setStatusOpen((v) => !v)}
      />

      <div className="bg-white rounded-2xl border border-(--gray-200) px-5 py-4">
        <div className="overflow-x-auto -mx-5 px-5">
          <table className="w-full min-w-190 border-collapse">
            <thead>
              <tr className="border-b border-(--gray-100)">
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-left pb-2">
                  Instructor
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-right pb-2">
                  Courses
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-right pb-2">
                  Students
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-right pb-2">
                  Rating
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-right pb-2 pr-6">
                  Revenue
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-left pb-2 pl-3 pr-3">
                  Status
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-left pb-2">
                  Joined
                </th>
                <th className="text-[11px] font-semibold tracking-widest text-(--gray-400) uppercase text-right pb-2">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-(--gray-50)">
              {pageRows.map((i) => (
                <tr
                  key={i.id}
                  className="hover:bg-(--gray-50)   transition-colors"
                >
                  <td className="py-3 pr-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-8 h-8 rounded-full shrink-0 bg-(--primary-50) text-(--primary-600) flex items-center justify-center text-[11px] font-semibold">
                        {i.initials}
                      </div>
                      <div className="min-w-0">
                        <p className="text-[13px] font-semibold text-(--text-title) truncate">
                          {i.name}
                        </p>
                        <p className="text-[11px] text-(--gray-400) truncate">
                          {i.email}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td className="py-3 pr-3 text-[13px] text-(--gray-600) text-right">
                    {i.courses}
                  </td>
                  <td className="py-3 pr-3 text-[13px] text-(--gray-600) text-right">
                    {i.students > 0 ? i.students.toLocaleString() : "—"}
                  </td>
                  <td className="py-3 pr-3 text-right">
                    {i.rating > 0 ? (
                      <span className="inline-flex items-center gap-1 text-[13px] font-medium text-(--text-title)">
                        <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                        {i.rating.toFixed(1)}
                      </span>
                    ) : (
                      <span className="text-[13px] text-(--gray-400)">—</span>
                    )}
                  </td>
                  <td className="py-3 pr-6 text-[13px] text-(--text-title) text-right font-medium">
                    {i.revenue}
                  </td>
                  <td className="py-3 pl-3 pr-3">
                    <span
                      className={`inline-block text-[11px] font-semibold px-2.5 py-1 rounded-full ${STATUS_BADGE[i.status]}`}
                    >
                      {i.status}
                    </span>
                  </td>
                  <td className="py-3 pr-3 text-[13px] text-(--gray-600) whitespace-nowrap">
                    {i.joined}
                  </td>
                  <td className="py-3 text-right">
                    <RowActionsMenu
                      instructor={i}
                      open={openMenuId === i.id}
                      onToggle={() =>
                        setOpenMenuId(openMenuId === i.id ? null : i.id)
                      }
                      setRef={(el) => menuRefs.current.set(i.id, el)}
                      onView={(instructor) =>
                        console.log("view", instructor.id)
                      }
                      onVerify={(instructor) =>
                        console.log("verify", instructor.id)
                      }
                      onToggleSuspend={(instructor) =>
                        console.log("toggle-suspend", instructor.id)
                      }
                      onDelete={(instructor) =>
                        console.log("delete", instructor.id)
                      }
                    />
                  </td>
                </tr>
              ))}
              {pageRows.length === 0 && (
                <tr>
                  <td
                    colSpan={8}
                    className="py-8 text-center text-[13px] text-(--gray-400)"
                  >
                    No instructors match your filters.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>

        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-4 pt-4 border-t border-(--gray-100)">
          <p className="text-[12px] text-(--gray-400)">
            Showing{" "}
            {filtered.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1}
            {"–"}
            {Math.min(currentPage * PAGE_SIZE, filtered.length)} of{" "}
            {filtered.length} instructors
          </p>
          <Pagination
            currentPage={currentPage}
            totalPages={totalPages}
            onPageChange={(p) => {
              setPage(p);
              setOpenMenuId(null);
            }}
          />
        </div>
      </div>
    </div>
  );
}
