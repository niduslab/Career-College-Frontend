...
import { MoreHorizontal, Pencil, Eye, Trash2, Send, Copy } from "lucide-react";
...
export default function ProposalActionMenu({ open, onToggle, setRef }: ActionMenuProps) {
  const btnRef = useRef<HTMLButtonElement | null>(null);
  const [coords, setCoords] = useState({ top: 0, right: 0 });

  const MENU_HEIGHT = 190; // 5 items × ~38px
