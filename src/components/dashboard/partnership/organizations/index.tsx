...
import OrganizationsStatsCards from "./stats-cards";
import OrganizationsTable from "./table";
import { TOP_ORGS, INDUSTRY_TIPS, ORGANIZATIONS } from "./data";
...
export default function OrganizationsPageContent() {
  ...
  const maxRevenue = Math.max(
    ...TOP_ORGS.map((o) => parseFloat(o.totalRevenue.replace(/[$,]/g, ""))),
  );
      ...
      <div className="w-full xl:w-60 2xl:w-72 shrink-0 space-y-4">
        ...
        <div className="bg-white border border-(--gray-200) rounded-2xl px-5 py-4 space-y-3">
          ...
          <div className="space-y-3">
            {TOP_ORGS.map((o, i) => {
              const val = parseFloat(o.totalRevenue.replace(/[$,]/g, ""));
              ...
              return (
                <div key={o.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <p className="text-[12px] font-medium text-(--text-title) truncate">{o.name}</p>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[12px] text-(--gray-500) shrink-0">{o.totalRevenue}</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="bg-white border border-(--gray-200) rounded-2xl px-5 py-4 space-y-3">
          ...
          <div className="space-y-2">
            {(["Active", "Pending", "Inactive"] as OrgStatus[]).map((st, i) => {
              const count = ORGANIZATIONS.filter((o) => o.status === st).length;
              const pct = Math.round((count / ORGANIZATIONS.length) * 100);
              const bar = st === "Active" ? "bg-green-500" : st === "Pending" ? "bg-orange-400" : "bg-gray-400";
              const text = st === "Active" ? "text-green-600" : st === "Pending" ? "text-orange-500" : "text-gray-500";
              return (
