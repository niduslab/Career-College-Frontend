...
import {
  ...
  type PublicInstitutionListItem,
...
const INSTITUTION_TYPE_FILTER_OPTIONS = [
...
export function InstitutionsGridSection() {
  const [institutions, setInstitutions] = useState<PublicInstitutionListItem[]>(
    [],
  );
  ...
  useEffect(() => {
    ...
    browsePublicInstitutions({
      ...
      .catch((err) => {
        if (!active) return;
        setError(
          err instanceof ApiError ? err.detail : "Failed to load institutions.",
        );
      })
      .finally(() => {
  ...
  }, [country, institutionType]);

  const handleLoadMore = async () => {
    setLoadingMore(true);
    try {
      const res = await browsePublicInstitutions({
        page: page + 1,
        page_size: PAGE_SIZE,
        country: country || undefined,
        institution_type: institutionType || undefined,
      });
      setInstitutions((prev) => [...prev, ...res.results]);
      setHasNext(Boolean(res.next));
      setPage((p) => p + 1);
    } catch (err) {
      setError(
        err instanceof ApiError
          ? err.detail
          : "Failed to load more institutions.",
      );
    } finally {
      ...
      <div className="mx-auto w-full max-w-310 px-4 md:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <p className="mt-10 text-center text-[14px] text-(--gray-400)">
            No public institution profiles found.
          </p>
        ) : (
          <>
            <div className="mt-8 grid gap-x-5 gap-y-8 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:mt-10 lg:mt-12">
              {institutions.map((institution) => {
                ...
                return (
                  <article key={institution.slug} className="group">
                    <Link
                      href={`/all-institutions-details/${institution.slug}`}
                    >
