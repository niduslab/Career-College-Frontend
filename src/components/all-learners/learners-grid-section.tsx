...
import {
  browsePublicLearners,
  EXPERIENCE_LEVELS,
  type PublicLearnerListItem,
...
export function LearnersGridSection() {
  const [learners, setLearners] = useState<PublicLearnerListItem[]>([]);
  ...
  useEffect(() => {
    ...
    browsePublicLearners({
      ...
      .catch((err) => {
        if (!active) return;
        setError(
          err instanceof ApiError ? err.detail : "Failed to load learners.",
        );
      })
      .finally(() => {
  ...
  const handleLoadMore = async () => {
    setLoadingMore(true);
    try {
      const res = await browsePublicLearners({
        page: page + 1,
        page_size: PAGE_SIZE,
        country: country || undefined,
        experience_level: experienceLevel || undefined,
      });
      setLearners((prev) => [...prev, ...res.results]);
      setHasNext(Boolean(res.next));
      setPage((p) => p + 1);
    } catch (err) {
      setError(
        err instanceof ApiError ? err.detail : "Failed to load more learners.",
      );
    } finally {
      ...
      <div className="mx-auto w-full max-w-310 px-4 md:px-6 lg:px-8">
        ...
        {loading ? (
          ...
          <p className="mt-10 text-center text-[14px] text-(--gray-400)">
            No public learner profiles found.
          </p>
        ) : (
          <>
            <div className="mt-8 grid gap-x-5 gap-y-8 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 md:mt-10 lg:mt-12">
              {learners.map((learner) => {
                ...
                return (
                  <article key={learner.slug} className="group">
                    <Link href={`/all-learners-details/${learner.slug}`}>
